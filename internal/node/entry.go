...
// name, enabled status, relay-only flag, and the tags for that node in that subscription.
// Returns ok=false if the subscription does not exist.
type SubLookupFunc func(subID string, hash Hash) (name string, enabled bool, relayOnly bool, tags []string, ok bool)
...
type NodeEntry struct {
	...
	DependencyBundle json.RawMessage
	CreatedAt        time.Time
...
func (e *NodeEntry) MatchRegexs(regexes []*regexp.Regexp, subLookup SubLookupFunc) bool {
	...
	if len(regexes) == 0 {
		for _, subID := range subs {
			_, enabled, _, _, ok := subLookup(subID, e.Hash)
			if ok && enabled {
				return true
			}
		}
	...
	for _, subID := range subs {
		name, enabled, _, tags, ok := subLookup(subID, e.Hash)
		if !ok || !enabled {
			continue
		}
...
// IsRelayOnly returns true when ALL subscriptions holding this node are
// relay-only. A node that belongs to at least one non-relay-only subscription
// is considered routable. Returns true (exclude) when subLookup is nil or
// when no valid subscriptions are found.
func (e *NodeEntry) IsRelayOnly(subLookup SubLookupFunc) bool {
	if subLookup == nil {
		return false
	}
	...
	for _, subID := range subs {
		_, enabled, relayOnly, _, ok := subLookup(subID, e.Hash)
		if !ok || !enabled {
			continue
		}
		if !relayOnly {
			return false // at least one non-relay-only sub → node is routable
		}
	}
	return true // all enabled subs are relay-only (or none were valid)
}
