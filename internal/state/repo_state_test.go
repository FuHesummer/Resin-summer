...
func TestMigrateStateDB_LegacyBaselineAdvancesToLatest(t *testing.T) {
	...
	`)
	if err != nil {
		t.Fatalf("create legacy latest-like tables: %v", err)
	}
	...
	if version != stateVersionAddRelayOnly {
		t.Fatalf("schema_migrations version: got %d, want %d", version, stateVersionAddRelayOnly)
	}
}
