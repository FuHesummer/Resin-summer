...
func (r *CacheRepo) BulkUpsertNodesStatic(nodes []model.NodeStatic) error {
	return bulkExecRows(
		...
		func(stmt *sql.Stmt, n model.NodeStatic) error {
			_, err := stmt.Exec(n.Hash, string(n.RawOptions), string(n.DependencyBundle), n.CreatedAtNs)
			return err
		},
	)
}
...
func (r *CacheRepo) LoadAllNodesStatic() ([]model.NodeStatic, error) {
	rows, err := r.db.Query("SELECT hash, raw_options_json, dependency_bundle_json, created_at_ns FROM nodes_static")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var dependencyBundleJSON string
		if err := rows.Scan(&n.Hash, &rawOptionsJSON, &dependencyBundleJSON, &n.CreatedAtNs); err != nil {
			return nil, err
		}
		n.RawOptions = json.RawMessage(rawOptionsJSON)
		n.DependencyBundle = json.RawMessage(dependencyBundleJSON)
...
func (r *CacheRepo) FlushTx(ops FlushOps) error {
	...
	steps := []struct {
		...
		{"upsert_nodes_static", upsertNodesStaticSQL, len(ops.UpsertNodesStatic), func(s *sql.Stmt, i int) error {
			...
			_, err := s.Exec(n.Hash, string(n.RawOptions), string(n.DependencyBundle), n.CreatedAtNs)
			return err
		}},
...
const (
	upsertNodesStaticSQL = `INSERT INTO nodes_static (hash, raw_options_json, dependency_bundle_json, created_at_ns)
		 VALUES (?, ?, ?, ?)
		 ON CONFLICT(hash) DO UPDATE SET
			raw_options_json       = excluded.raw_options_json,
			dependency_bundle_json = excluded.dependency_bundle_json,
