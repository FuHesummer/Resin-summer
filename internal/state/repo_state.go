...
func (r *StateRepo) UpsertSubscription(s model.Subscription) error {
	...
	_, err := r.db.Exec(`
		INSERT INTO subscriptions (id, name, source_type, url, content, update_interval_ns, enabled,
		                           ephemeral, relay_only, ephemeral_node_evict_delay_ns, created_at_ns, updated_at_ns)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(id) DO UPDATE SET
			name               = excluded.name,
	...
	`, s.ID, s.Name, s.SourceType, s.URL, s.Content, s.UpdateIntervalNs, s.Enabled,
		s.Ephemeral, s.RelayOnly, s.EphemeralNodeEvictDelayNs, s.CreatedAtNs, s.UpdatedAtNs)
	return err
}
...
func (r *StateRepo) ListSubscriptions() ([]model.Subscription, error) {
	rows, err := r.db.Query(`SELECT id, name, source_type, url, content, update_interval_ns, enabled,
		ephemeral, relay_only, ephemeral_node_evict_delay_ns, created_at_ns, updated_at_ns FROM subscriptions`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.Name, &s.SourceType, &s.URL, &s.Content, &s.UpdateIntervalNs, &s.Enabled,
			&s.Ephemeral, &s.RelayOnly, &s.EphemeralNodeEvictDelayNs, &s.CreatedAtNs, &s.UpdatedAtNs); err != nil {
			return nil, err
		}
		if s.SourceType == "" {
			s.SourceType = "remote"
