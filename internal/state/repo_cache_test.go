...
func TestCacheRepo_NodesStatic_BulkUpsertAndLoad(t *testing.T) {
	...
	nodes := []model.NodeStatic{
		{
			Hash:             "aaa",
			RawOptions:       json.RawMessage(`{"type":"ss"}`),
			DependencyBundle: json.RawMessage(`{"root":"a","deps":["x"]}`),
			CreatedAtNs:      100,
		},
		{
			Hash:             "bbb",
			RawOptions:       json.RawMessage(`{"type":"vmess"}`),
	...
	nodes[0].DependencyBundle = json.RawMessage(`{"root":"updated"}`)
	...
	for _, n := range loaded {
		if n.Hash == "aaa" {
			if string(n.RawOptions) != `{"type":"ss","updated":true}` {
				t.Fatalf("expected updated options, got %s", string(n.RawOptions))
			}
			if string(n.DependencyBundle) != `{"root":"updated"}` {
				t.Fatalf("expected updated dependency bundle, got %s", string(n.DependencyBundle))
			}
		}
	}
}
