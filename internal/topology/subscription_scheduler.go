...
func (s *SubscriptionScheduler) UpdateSubscription(sub *subscription.Subscription) {
	// Snapshot the current LastUpdatedNs as the baseline for this attempt.
	// Using LastUpdatedNs (instead of time.Now()) makes the stale-guard
	// immune to coarse clock resolution (e.g. ~15 ms on Windows): a
	// sequential second call reads the value stored by the first, and
	// inside the lock LastUpdatedNs is bumped to be strictly greater,
	// so the guard ("> baseline") correctly passes for the second call.
	// Concurrent attempts that start before any success will read the
	// same baseline, and the loser will be discarded because the winner
	// bumps LastUpdatedNs above that shared baseline.
	attemptStartedNs := sub.LastUpdatedNs.Load()
	attemptURL := sub.URL()
	...
	depByHash := make(map[node.Hash][]byte) // DependencyBundle per hash
	for _, p := range parsed {
		...
		if len(p.DependencyBundle) > 0 {
			depByHash[h] = p.DependencyBundle
		}
	}
	...
	sub.WithOpLock(func() {
		...
		for _, h := range added {
			s.pool.AddNodeFromSub(h, rawByHash[h], depByHash[h], sub.ID)
		}
		for _, h := range kept {
		...
		// LastUpdatedNs must be strictly greater than attemptStartedNs so
		// that the stale-guard ("LastUpdatedNs > attemptStartedNs")
		// correctly discards any concurrent older attempt whose start
		// time <= this attempt's start time. On systems with coarse
		// clock resolution (e.g. ~15 ms on Windows) time.Now() may
		// return the same nanosecond as attemptStartedNs, so we bump
		// by 1 ns when needed.
		now := time.Now().UnixNano()
...
// SetSubscriptionRelayOnly updates the relay_only flag and rebuilds all platform
// routable views. Setting relay_only=true excludes all nodes owned exclusively
// by this subscription from platform routing; they remain in the pool for
// relay/detour chain support only.
func (s *SubscriptionScheduler) SetSubscriptionRelayOnly(sub *subscription.Subscription, relayOnly bool) {
	sub.WithOpLock(func() {
		sub.SetRelayOnly(relayOnly)
	})
	// Rebuild all platform views so they pick up the relay-only change.
	s.pool.RebuildAllPlatforms()
	if s.onSubUpdated != nil {
		s.onSubUpdated(sub)
	}
...
func (s *SubscriptionScheduler) RenameSubscription(sub *subscription.Subscription, newName string) {
	sub.WithOpLock(func() {
		...
		sub.ManagedNodes().RangeNodes(func(h node.Hash, managed subscription.ManagedNode) bool {
			...
			if ok {
				s.pool.AddNodeFromSub(h, entry.RawOptions, entry.DependencyBundle, sub.ID)
			}
			return true
		})
	})
