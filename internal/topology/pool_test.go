...
func TestPool_AddNodeFromSub_Idempotent(t *testing.T) {
	...
	sub := subscription.NewSubscription("s1", "Sub1", "url", true, false, false)
	subMgr.Register(sub)
...
func TestPool_AddNodeFromSub_ReAddDoesNotResetCircuitOpenSince(t *testing.T) {
	subMgr := NewSubscriptionManager()
	sub1 := subscription.NewSubscription("s1", "Sub1", "url", true, false, false)
	sub2 := subscription.NewSubscription("s2", "Sub2", "url", true, false, false)
	...
	pool.AddNodeFromSub(h, raw, nil, "s2")
...
func TestPool_ConcurrentAddRemove(t *testing.T) {
	subMgr := NewSubscriptionManager()
	for i := 0; i < 10; i++ {
		sub := subscription.NewSubscription(fmt.Sprintf("s%d", i), fmt.Sprintf("Sub%d", i), "url", true, false, false)
		subMgr.Register(sub)
	}
	...
	for g := 0; g < 10; g++ {
		...
		go func(subIdx int) {
			...
			for i := subIdx * 10; i < (subIdx+1)*10; i++ {
				pool.AddNodeFromSub(hashes[i], raws[i], nil, subID)
			}
		}(g)
	}
	wg.Wait()
...
func TestPool_RegexFilteredPlatform(t *testing.T) {
	subMgr := NewSubscriptionManager()
	sub := subscription.NewSubscription("s1", "Provider", "url", true, false, false)
	subMgr.Register(sub)
	...
	for _, h := range []node.Hash{h1, h2} {
		pool.AddNodeFromSub(h, nil, nil, "s1")
		entry, _ := pool.GetEntry(h)
		entry.LatencyTable.LoadEntry("example.com", node.DomainLatencyStats{
			Ewma:        100 * time.Millisecond,
			LastUpdated: time.Now(),
...
func TestPool_ResolveNodeDisplayTag_EarliestSubscriptionThenMinTag(t *testing.T) {
	...
	older := subscription.NewSubscription("sub-old", "Z-Provider", "url", true, false, false)
	older.CreatedAtNs = 100
	older.SetEnabled(false) // display-tag rule should not depend on enabled flag.

	newer := subscription.NewSubscription("sub-new", "A-Provider", "url", true, false, false)
	...
	pool.AddNodeFromSub(h, raw, nil, older.ID)
	pool.AddNodeFromSub(h, raw, nil, newer.ID)
...
func TestEphemeralCleaner_EvictsCircuitBroken(t *testing.T) {
	subMgr := NewSubscriptionManager()
	sub := subscription.NewSubscription("s1", "EphSub", "url", true, true, false) // ephemeral
...
func TestEphemeralCleaner_SkipsNonEphemeral(t *testing.T) {
	subMgr := NewSubscriptionManager()
	sub := subscription.NewSubscription("s1", "RegularSub", "url", true, false, false) // NOT ephemeral
	subMgr.Register(sub)
...
func TestEphemeralCleaner_SkipsRecentCircuitBreak(t *testing.T) {
	subMgr := NewSubscriptionManager()
	sub := subscription.NewSubscription("s1", "EphSub", "url", true, true, false)
	sub.SetEphemeralNodeEvictDelayNs(int64(1 * time.Minute))
	subMgr.Register(sub)
