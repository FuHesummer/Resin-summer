...
func TestAPIContract_GetLease_IncludesNodeTag(t *testing.T) {
	...
	older := subscription.NewSubscription("sub-old", "Z-Provider", "https://example.com/sub-old", true, false, false)
	older.CreatedAtNs = 100
	olderManaged := subscription.NewManagedNodes()

	newer := subscription.NewSubscription("sub-new", "A-Provider", "https://example.com/sub-new", true, false, false)
	...
	cp.Pool.AddNodeFromSub(hash, raw, nil, older.ID)
	cp.Pool.AddNodeFromSub(hash, raw, nil, newer.ID)
...
func TestAPIContract_PlatformListIncludesRoutableNodeCount(t *testing.T) {
	...
	cp.SubMgr.Register(subscription.NewSubscription("sub-test", "sub-test", "https://example.com/sub", true, false, false))

	raw := []byte(`{"type":"ss","server":"1.1.1.1","port":443}`)
	hash := node.HashFromRawOptions(raw)
	cp.Pool.AddNodeFromSub(hash, raw, nil, "sub-test")
