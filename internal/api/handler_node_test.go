...
func addNodeForNodeListTestWithTag(
	...
	cp.Pool.AddNodeFromSub(hash, []byte(raw), nil, sub.ID)
...
func TestHandleListNodes_TagKeywordFiltersByNodeName(t *testing.T) {
	...
	subA := subscription.NewSubscription("11111111-1111-1111-1111-111111111111", "sub-a", "https://example.com/a", true, false, false)
...
func TestHandleListNodes_UniqueEgressIPsUsesFilteredResult(t *testing.T) {
	...
	subA := subscription.NewSubscription("11111111-1111-1111-1111-111111111111", "sub-a", "https://example.com/a", true, false, false)
	subB := subscription.NewSubscription("22222222-2222-2222-2222-222222222222", "sub-b", "https://example.com/b", true, false, false)
	cp.SubMgr.Register(subA)
...
func TestHandleProbeEgress_ReturnsRegion(t *testing.T) {
	...
	sub := subscription.NewSubscription("11111111-1111-1111-1111-111111111111", "sub-a", "https://example.com/a", true, false, false)
	cp.SubMgr.Register(sub)

	raw := []byte(`{"type":"ss","server":"1.1.1.1","port":443}`)
	hash := node.HashFromRawOptions(raw)
	cp.Pool.AddNodeFromSub(hash, raw, nil, sub.ID)
	sub.ManagedNodes().StoreNode(hash, subscription.ManagedNode{Tags: []string{"tag"}})
