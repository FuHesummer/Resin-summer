...
//     (url/updateInterval/name/enabled/ephemeral/relayOnly/ephemeralNodeEvictDelayNs).
...
func NewSubscription(id, name, url string, enabled, ephemeral, relayOnly bool) *Subscription {
	s := &Subscription{
		ID:                        id,
		url:                       url,
...
// RelayOnly returns whether the subscription is relay-only (thread-safe).
// Relay-only subscriptions contribute nodes to the pool for relay/detour chain
// usage but those nodes are excluded from platform routable views.
func (s *Subscription) RelayOnly() bool {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.relayOnly
}

// SetRelayOnly updates the relay-only flag (thread-safe).
func (s *Subscription) SetRelayOnly(v bool) {
	s.mu.Lock()
	s.relayOnly = v
	s.mu.Unlock()
}
