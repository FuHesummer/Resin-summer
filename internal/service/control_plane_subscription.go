...
func (s *ControlPlaneService) CreateSubscription(req CreateSubscriptionRequest) (*SubscriptionResponse, error) {
	...
	sub := subscription.NewSubscription(id, name, subURL, enabled, ephemeral, relayOnly)
...
func (s *ControlPlaneService) UpdateSubscription(id string, patchJSON json.RawMessage) (*SubscriptionResponse, error) {
	...
	newRelayOnly := sub.RelayOnly()
	relayOnlyChanged := false
	if b, ok, err := patch.optionalBool("relay_only"); err != nil {
		return nil, err
	} else if ok {
		if b != newRelayOnly {
			relayOnlyChanged = true
		}
		newRelayOnly = b
	}
	...
	if relayOnlyChanged {
		s.Scheduler.SetSubscriptionRelayOnly(sub, newRelayOnly)
	}
