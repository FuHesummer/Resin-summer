...
func TestCleanupSubscriptionCircuitOpenNodes_RemovesCircuitAndOutboundFailureNodes(t *testing.T) {
	...
	subA := subscription.NewSubscription("sub-a", "sub-a", "https://example.com/a", true, false, false)
	subB := subscription.NewSubscription("sub-b", "sub-b", "https://example.com/b", true, false, false)
	...
	pool.AddNodeFromSub(circuitHash, circuitRaw, nil, subA.ID)
	...
	pool.AddNodeFromSub(noOutboundErrorHash, noOutboundErrorRaw, nil, subA.ID)
	...
	pool.AddNodeFromSub(healthyHash, healthyRaw, nil, subA.ID)
	...
	pool.AddNodeFromSub(sharedHash, sharedRaw, nil, subA.ID)
	pool.AddNodeFromSub(sharedHash, sharedRaw, nil, subB.ID)
...
func TestCleanupSubscriptionCircuitOpenNodes_SecondConfirmSkipsRecoveredNodes(t *testing.T) {
	...
	sub := subscription.NewSubscription("sub-a", "sub-a", "https://example.com/a", true, false, false)
	subMgr.Register(sub)

	recoveredRaw := []byte(`{"type":"ss","server":"5.5.5.5","port":443}`)
	recoveredHash := node.HashFromRawOptions(recoveredRaw)
	pool.AddNodeFromSub(recoveredHash, recoveredRaw, nil, sub.ID)
	...
	pool.AddNodeFromSub(failedHash, failedRaw, nil, sub.ID)
