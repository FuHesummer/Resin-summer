...
func TestChooseSameIPRotationCandidate_PicksLowestLatency(t *testing.T) {
	...
	plat.NotifyDirty(
		currentHash,
		pool.GetEntry,
		func(_ string, _ node.Hash) (string, bool, bool, []string, bool) { return "", true, false, nil, true },
...
func TestRouteRequest_SameIPRotationMissRecreatesLease(t *testing.T) {
	...
	// Use a far-future, distinguishable expiry so the assertion works even on
	// systems with coarse clock resolution (e.g. ~15 ms on Windows) where
	// time.Now() in the test and time.Now() inside createLease may return
	// the same nanosecond. The value must still be in the future so the
	// lease is NOT treated as expired (the test exercises the circuit-open
	// rotation path, not the TTL-expiry path).
