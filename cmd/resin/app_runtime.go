...
func (a *resinApp) buildNetworkServers(engine *state.StateEngine) error {
	...
	// --- SOCKS5 inbound on the same port (auto-detected by first byte) ---
	...
	// Wrap the listener with protocol detection: SOCKS5 (0x05) is handled
	// directly; everything else is passed through to the HTTP server.
	a.inboundLn = proxy.NewProtocolMuxListener(a.inboundLn, a.socks5Srv)
...
func (a *resinApp) startServers() <-chan error {
	...
	go func() {
		log.Printf("Resin server starting on %s (HTTP + SOCKS5)", formatListenURL(a.envCfg.ListenAddress, a.envCfg.ResinPort))
