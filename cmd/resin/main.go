...
func bootstrapTopology(
	...
	for _, ms := range dbSubs {
		sub := subscription.NewSubscription(ms.ID, ms.Name, ms.URL, ms.Enabled, ms.Ephemeral, ms.RelayOnly)
	...
	return state.CacheReaders{
		ReadNodeStatic: func(hash string) *model.NodeStatic {
			...
			return &model.NodeStatic{
				...
				DependencyBundle: append(json.RawMessage(nil), entry.DependencyBundle...),
...
func loadBootstrapNodeStatics(
	...
	for _, ns := range statics {
		...
		entry := &node.NodeEntry{
			...
			DependencyBundle: append(json.RawMessage(nil), ns.DependencyBundle...),
