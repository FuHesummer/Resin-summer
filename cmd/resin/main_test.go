...
func TestBootstrapNodes_MissingDynamicDefaultsCircuitOpen(t *testing.T) {
	...
	depBundle := json.RawMessage(`{"root":{"tag":"root"},"dependencies":[{"tag":"dep-1"}]}`)
	hash := node.HashFromRawOptions(raw)
	if err := engine.BulkUpsertNodesStatic([]model.NodeStatic{{
		Hash:             hash.Hex(),
		RawOptions:       raw,
		DependencyBundle: depBundle,
	...
	if string(entry.DependencyBundle) != string(depBundle) {
		t.Fatalf("DependencyBundle mismatch after bootstrap: got %s, want %s", string(entry.DependencyBundle), string(depBundle))
	}
...
func TestBootstrapNodes_DynamicRecordOverridesDefaultCircuitOpen(t *testing.T) {
	...
	depBundle := json.RawMessage(`{"root":{"tag":"root-dyn"},"dependencies":[{"tag":"dep-dyn"}]}`)
	hash := node.HashFromRawOptions(raw)
	if err := engine.BulkUpsertNodesStatic([]model.NodeStatic{{
		Hash:             hash.Hex(),
		RawOptions:       raw,
