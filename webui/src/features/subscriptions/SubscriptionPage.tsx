...
const SUBSCRIPTION_RELAY_ONLY_HINT = "仅中转订阅的节点不会加入代理池，只作为链式代理的中转节点使用。";
...
function subscriptionToEditForm(subscription: Subscription): SubscriptionEditForm {
  return {
    ...
    relay_only: subscription.relay_only,
  };
}
  ...
  const updateMutation = useMutation({
    mutationFn: async (formData: SubscriptionEditForm) => {
      ...
      const payload = {
        ...
        relay_only: formData.relay_only,
        ...
        cell: (info) => {
          ...
          return (
            <div className="subscriptions-status-cell">
              ...
              {s.relay_only ? (
                <Badge variant="info">{t("仅中转")}</Badge>
              ) : null}
            </div>
          );
        },
            ...
            <div className="platform-drawer-layout">
              <section className="platform-drawer-section">
                ...
                <form className="form-grid" onSubmit={onEditSubmit}>
                  ...
                  <div className="field-group">
                    <label className="field-label" htmlFor="edit-sub-relay-only" style={{ visibility: "hidden" }}>
                      {t("仅中转")}
                    </label>
                    <div className="subscription-switch-item">
                      <label className="subscription-switch-label" htmlFor="edit-sub-relay-only">
                        <span>{t("仅中转")}</span>
                        <span
                          className="subscription-info-icon"
                          title={t(SUBSCRIPTION_RELAY_ONLY_HINT)}
                          aria-label={t(SUBSCRIPTION_RELAY_ONLY_HINT)}
                          tabIndex={0}
                        >
                          <Info size={13} />
                        </span>
                      </label>
                      <Switch id="edit-sub-relay-only" {...editForm.register("relay_only")} />
                    </div>
                  </div>
        ...
        <div className="modal-overlay" role="dialog" aria-modal="true">
          <Card className="modal-card">
            ...
            <form className="form-grid" onSubmit={onCreateSubmit}>
              ...
              <div className="field-group">
                <label className="field-label" htmlFor="create-sub-relay-only" style={{ visibility: "hidden" }}>
                  {t("仅中转")}
                </label>
                <div className="subscription-switch-item">
                  <label className="subscription-switch-label" htmlFor="create-sub-relay-only">
                    <span>{t("仅中转")}</span>
                    <span
                      className="subscription-info-icon"
                      title={t(SUBSCRIPTION_RELAY_ONLY_HINT)}
                  ...
                  <Switch id="create-sub-relay-only" {...createForm.register("relay_only")} />
                </div>
              </div>
